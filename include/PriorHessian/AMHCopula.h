...
class AMHCopula : public ArchimedeanCopula
{
    ...
    static const double _theta_ubound = std::nextafter(1,-1);//1-epsilon
    static const StringVecT _param_names = {std::string("theta")};
public:
...
private:
    ...
    template<class Vec>
    static double compute_pdf_terms(double theta, const Vec &u);
...
template<int Ndim>
const double AMHCopula<Ndim>::_default_theta;
template<int Ndim>
const double AMHCopula<Ndim>::_theta_lbound;
template<int Ndim>
const double AMHCopula<Ndim>::_theta_ubound;
template<int Ndim>
const StringVecT AMHCopula<Ndim>::_param_names;
...
{ 
    ...
    return sign_1mtheta/_theta * polylog<-Ndim>(z); 
}
...
{ 
    ...
    return sign_1mtheta/theta * polylog<-Ndim>(z); 
}

template<int Ndim>
template<class Vec>
typedef AMHCopula<Ndim>::PDFTerms 
...
void AMHCopula<Ndim>::compute_d_gen_terms(double theta, const Vec &u)
{
    D_GenTerms terms;
